...
def find_line(binary_warped):
    ...
    out_img = np.dstack((binary_warped, binary_warped, binary_warped))##########################################3
    ...
    # Colors in the left and right lane regions 滑块可视化
...
def draw_line(undist,binary_warped,Minv,left_fit, right_fit):
    ...
    # plot1 = plt.plot(left_x, l_y, 'red', linewidth=5.0)
    # plot2 = plt.plot(right_x, r_y, 'red', linewidth=5.0)
    # im = cv2.flip(plot1, 1);
    plt.show()
...
def roi(img,binary_warped,Minv,left_fit, right_fit):
    ...
    # (b, g, r) = cv2.split(Mask_inv)
    # g = cv2.split(Mask_inv)[1]
...
def calculate_curv_and_pos(binary_warped,left_fit, right_fit):
    ...
    ploty = np.linspace(0, binary_warped.shape[0]-1, binary_warped.shape[0] ) #在指定的间隔内返回均匀间隔的数字。
    leftx = left_fit[0]*ploty**2 + left_fit[1]*ploty + left_fit[2]
    rightx = right_fit[0]*ploty**2 + right_fit[1]*ploty + right_fit[2]
    # Define conversions in x and y from pixels space to meters
    ym_per_pix = 30/1080 # meters per pixel in y dimension
    xm_per_pix = 3/750 # meters per pixel in x dimension
    y_eval = np.max(ploty)
    # Fit new polynomials to x,y in world space
    left_fit_cr = np.polyfit(ploty*ym_per_pix, leftx*xm_per_pix, 2) #得到3个参数
...
def draw_values(img,curvature,distance_from_center):
    ...
    cv2.putText(img,radius_text,(100,100), font, 1,(255,255,255),2) #图像，文字内容， 坐标 ，字体，大小，颜色，字体厚度
